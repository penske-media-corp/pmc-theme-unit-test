...
class Taxonomies {
	...
	public function save_taxonomy( $taxonomy_json ) {
		...
		try {
			...
			if ( false === $taxonomy_id ) {
				...
				if ( in_array( $taxonomy_json['object_type'], (array) $built_in_posttypes, true ) ) {
	...
	public function instant_taxonomies_import( $taxonomies_json ) {
		...
		foreach ( $taxonomies_json as $taxonomy_json ) {
			...
			if ( in_array( $taxonomy_json['name'], (array) Config::$default_taxonomies, true ) ) {
				continue;
			}
